...
class SpreadDetailForSpreadCaptainController extends CommonController{

    protected $table = "Customer";


    public function index(){
        $this->assign("groups", $this->_getUsers());
        $this->display();
    }

    private function _getUsers(){
        $group_id = $this->getUserGroupId();
        return D("User")->getGroupEmployee($group_id, "id,realname as name");
    }
    ...
    private function setDepartEm(){
        $users = D("User")->getGroupEmployee($this->getUserGroupId(), 'id');
