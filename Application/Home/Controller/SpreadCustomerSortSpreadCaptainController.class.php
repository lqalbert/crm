...
class SpreadCustomerSortSpreadCaptainController extends SpreadCustomerSortController{
            ...
            case 'to_gid':
                if (!empty($this->spread_id)) {
                    ...
                    $re = D("Group")->join("left join department_basic as db on group_basic.department_id=db.id")
                              ->where(array('db.id'=>$this->spread_id,"group_basic.status"=>1,'group_basic.id'=> $this->group_id))
