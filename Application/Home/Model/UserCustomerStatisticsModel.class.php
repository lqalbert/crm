...
class UserCustomerStatisticsModel extends CustomerCountModel {
    ...
    */
    public function setTypeCount(){
        $sql = "select count(cb.id) as c , `type` , ui.user_id from customers_basic as cb inner join user_info as ui on cb.salesman_id= ui.user_id where ui.group_id =".$this->group_id." group by cb.type, ui.user_id";
        $re = M()->query($sql);
    ...
    */
    public function setVCount(){
        $sql = "select count(cus_id), ui.user_id from customers_service as cs inner join user_info as ui on cs.user_id = ui.user_id where time > '".$this->date['start']."' and time <'".$this->date['end']."' and ui.group_id=".$this->group_id." group by ui.user_id";
