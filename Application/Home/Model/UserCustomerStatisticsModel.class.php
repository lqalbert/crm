...
class UserCustomerStatisticsModel extends CustomerCountModel {
    ...
    */
    public function setTypeCount(){
        $sql = "select count(cb.id) as c , `type` , ui.user_id from customers_basic as cb inner join user_info as ui on cb.salesman_id= ui.user_id where ui.user_id =".$this->group_id." group by ui.user_id";
        $re = M()->query($sql);
