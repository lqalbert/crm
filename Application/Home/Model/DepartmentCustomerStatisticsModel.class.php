...
class DepartmentCustomerStatisticsModel extends CustomerCountModel {
    ...
    */
    public function setTypeCount(){
        $sql = "select count(cb.id) as c , `type` , department_id from customers_basic as cb inner join user_info as ui on cb.salesman_id= ui.user_id where cb.status=1  group by cb.type, ui.department_id";
        $re = M()->query($sql);
    ...
    */
    public function setVCount(){
        $sql = "select count(cus_id) as c, department_id from customers_order as cs inner join user_info as ui on cs.user_id = ui.user_id where created_at > '".$this->date['start']."' and created_at <'".$this->date['end']."' group by department_id";
