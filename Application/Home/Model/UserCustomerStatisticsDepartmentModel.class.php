...
class UserCustomerStatisticsDepartmentModel extends CustomerCountModel {
    ...
    */
    public function setTypeCount(){
     ...
     public function setConflictCount(){
        ...
        $sql = "select count(id) as c , ui.user_id , group_id from customers_conflict as cc inner join user_info as ui on cc.user_id = ui.user_id where cc.created > '".$this->date['start']."' and created < '".$this->date['end']."' and department_id=".$this->department_id."  group by ui.user_id ";
    ...
    public function setConflictedCount(){
        $sql = "select count(customers_conflict.id) as c , ui.user_id , group_id  from customers_basic as cb inner join customers_conflict on cb.id = customers_conflict.cus_id inner join user_info as ui on cb.salesman_id = ui.user_id where customers_conflict.created > '".$this->date['start']."' and customers_conflict.created <'".$this->date['end']."'  and department_id=".$this->department_id."  group by ui.user_id ";
        $re = M()->query($sql);
        foreach ($re as  $value) {
    ...
    public function setPullCount(){
        $sql = "select count(cus_id)as c, ui.user_id , group_id from customers_pulls as cp inner join user_info as ui on cp.to_id = ui.user_id where created_at > '".$this->date['start']."' and created_at <'".$this->date['end']."' and department_id=".$this->department_id." group by ui.user_id";
        $re = M()->query($sql);
        foreach ($re as  $value) {
    ...
    */
    public function setVCount(){
        $sql = "select count(cus_id) as c, ui.user_id, group_id from customers_order as cs inner join user_info as ui on cs.user_id = ui.user_id where created_at > '".$this->date['start']."' and created_at <'".$this->date['end']."' and department_id=".$this->department_id." group by ui.user_id";
